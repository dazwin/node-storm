...
function shellcomponent(args) {
	args = args || {};

	return new ttypes.ComponentObject({
		shell: new ttypes.ShellComponent({
			execution_command: args.execution_command || process.argv[0],
			script: args.script || util.topologyScript()
		})
	})
}
...
exports.shellbolt = function shellbolt(args) {
	return new ttypes.Bolt({
		bolt_object: shellcomponent(args),
		common: componentcommon()
	})
}

exports.shellspout = function shellspout(args) {
	return new ttypes.SpoutSpec({
		spout_object: shellcomponent(args),
		common: componentcommon()
	})
}
